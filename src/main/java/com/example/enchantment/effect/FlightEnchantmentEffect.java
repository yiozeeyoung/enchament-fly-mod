...
public record FlightEnchantmentEffect() implements EnchantmentEntityEffect {

    // 由于此效果目前没有从JSON中读取的参数，我们使用 MapCodec.unit()
    // 它会调用 FlightEnchantmentEffect 的无参构造函数。
    public static final MapCodec<FlightEnchantmentEffect> CODEC = MapCodec.unit(FlightEnchantmentEffect::new);
    ...
    @Override
    public MapCodec<FlightEnchantmentEffect> getCodec() { // 注意：这里返回具体的类型 MapCodec<FlightEnchantmentEffect>
        return CODEC;
    }
}
